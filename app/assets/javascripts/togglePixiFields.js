...
function toggleFields(ctype) {
  if(ctype.match(/^event/) != null) {
    ...
    toggleReqFlds('#et_code', '#cond-type-fld, #yr_built', '#cond-type-fld');
    hideComp(true);
  }
  else {
    ...
    if(ctype.match(/^employment/) != null) {
      ...
      toggleReqFlds('#temp_listing_job_type_code', '#cond-type-fld, #yr_built', '#cond-type-fld, #yr_built');
      toggleYear(ctype);
    }
    else {
      hideComp(false);
      if(ctype.match(/^service/) != null) {
        ...
        toggleReqFlds('', '#cond-type-fld, #yr_built', '#cond-type-fld, #yr_built');
      }

      if(ctype.match(/^sales/) != null || ctype.match(/^asset/) != null) {
        refreshPage('#vehicle-fields, #product-fields', '#cond-type-fld, #qty-fld', true);
        toggleReqFlds('#cond-type-fld', '#yr_built', '#temp_listing_other_id, #temp_listing_mileage, #temp_listing_item_size, #temp_listing_color');
      }

      if(ctype.match(/^vehicle/) != null) {
        refreshPage('#qty-fld, #product-fields', '#vehicle-fields, #cond-type-fld', true);
        toggleReqFlds('#cond-type-fld, #yr_built', '', '#qty-fld, #temp_listing_other_id, #temp_listing_item_size');
      }

      if(ctype.match(/^product/) != null) {
        console.log('in product type');
        refreshPage('#vehicle-fields', '#product-fields, #cond-type-fld', true);
        toggleReqFlds('#cond-type-fld', '#yr_built', '#temp_listing_other_id, #temp_listing_mileage');
      }
      toggleYear(ctype);
    }
  }
